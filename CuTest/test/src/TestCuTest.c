...
void TestAssertBoolEquals(CuTest* tc)
{
	jmp_buf buf;
	CuTest *tc2 = CuTestNew("TestAssertBoolEquals", zTestFails);
	const char* expected = "expected <true> but was <false>";
	const char* expectedMsg = "some text: expected <true> but was <false>";
	tc2->jumpBuf = &buf;
	if (setjmp(buf) == 0)
	{
		CuAssertBoolEquals(tc2, true, false);
	}
	CuAssertTrue(tc, tc2->failed);
	CompareAsserts(tc, "CuAssertBoolEquals failed", expected, tc2->message);
	if (setjmp(buf) == 0)
	{
		CuAssertBoolEquals_Msg(tc2, "some text", true, false);
	}
	CuAssertTrue(tc, tc2->failed);
	CompareAsserts(tc, "CuAssertBoolEquals failed", expectedMsg, tc2->message);
}

void TestAssertCharEquals(CuTest* tc)
{
	jmp_buf buf;
	CuTest *tc2 = CuTestNew("TestAssertCharEquals", zTestFails);
	const char* expected = "expected <a> but was <b>";
	const char* expectedMsg = "some text: expected <a> but was <b>";
	tc2->jumpBuf = &buf;
	if (setjmp(buf) == 0)
	{
		CuAssertCharEquals(tc2, 'a', 'b');
	}
	CuAssertTrue(tc, tc2->failed);
	CompareAsserts(tc, "CuAssertCharEquals failed", expected, tc2->message);
	if (setjmp(buf) == 0)
	{
		CuAssertCharEquals_Msg(tc2, "some text", 'a', 'b');
	}
	CuAssertTrue(tc, tc2->failed);
	CompareAsserts(tc, "CuAssertCharEquals failed", expectedMsg, tc2->message);
}
...
void TestAssertIntEquals(CuTest* tc)
{
	...
	CuAssertTrue(tc, tc2->failed);
	CompareAsserts(tc, "CuAssertIntEquals failed", expectedMsg, tc2->message);
}
...
/********************************************/
/* Test Suite for CuTest library functions  */
/********************************************/
...
CuSuite* CuGetSuite(void)
{
	...
	SUITE_ADD_TEST(suite, TestAssertBoolEquals);
	SUITE_ADD_TEST(suite, TestAssertCharEquals);
